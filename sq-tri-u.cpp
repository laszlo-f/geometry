	//p=0, from above
		{
			...
			for(i=0;i<sl*sl;i++){
				if(i!=sl*sl/2+sl/2){
					...
					x=((long double)(i%sl));
					y=sqrtl(3.)*(i/sl);
					film.push_back(new particle(x,y,0,edge(i,sl)));
			...
			shapeu.open(("shape-s-u-p"+Ttos(prestr)+".txt").c_str(),ios::app);		
			forceu.open(("force-s-u-p"+Ttos(prestr)+".txt").c_str(),ios::app);		
			logforceu.open(("logforce-s-u-p"+Ttos(prestr)+".txt").c_str(),ios::app);		
