	//p=.1, from above
		{
			...
			for(i=0;i<sl*sl;i++){
				if(i!=sl*sl/2+sl/2){
					...
					x=((double)(i%sl));
					y=sqrtl(3.)*(i/sl);
					film.push_back(new particle(x,y,0, dist(x,y,sl)));
			...
			shapeu.open(("shape-c-u-p"+Ttos(prestr)+".txt").c_str(),ios::app);		
			forceu.open(("force-c-u-p"+Ttos(prestr)+".txt").c_str(),ios::app);		
			logforceu.open(("logforce-c-u-p"+Ttos(prestr)+".txt").c_str(),ios::app);		
			
			deleter(&film,.1);//monte carlo element
			clean(&film);
